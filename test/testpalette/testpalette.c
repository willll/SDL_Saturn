...
{
  SDL_LogCritical(SDL_LOG_CATEGORY_APPLICATION, "SDL error: %s\n",SDL_GetError());
  for(;;);
}
...
static SDL_Surface *make_bg(SDL_Surface *screen, int startcol)
{
  ...
  */
  ...
  {
    int i;
    SDL_Surface *z = SDL_CreateRGBSurface(SDL_HWSURFACE, s->w, s->h, 8,
      0, 0, 0, 0);
      /* copy palette */
...
int main(/*int argc, char **argv*/)
{
  ...
  SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,
                  "%s l%d :We've chosen magenta (#ff00ff) as colour key for the boat\n",
                __FUNCTION__,
                __LINE__);
  ...
  */
  SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,
                  "%s l%d :First set the physical screen palette to black, so the user won't see our initial drawing on the screen.\n",
                __FUNCTION__,
                __LINE__);
  ...
  */
  SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,
                  "%s l%d :Now the logical screen palette is set, and will remain unchanged. The boats already have the same palette so fast blits can be used.\n",
                __FUNCTION__,
                __LINE__);
  ...
  SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,
                  "%s l%d save the index of the red colour for later\n",
                __FUNCTION__,
                __LINE__);
  red = SDL_MapRGB(screen->format, 0xff, 0x00, 0x00);

  SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,
                  "%s l%d make a nice wavy background surface\n",
                __FUNCTION__,
                __LINE__);
  ...
  SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,
                  "%s l%d determine initial boat placements\n",
                __FUNCTION__,
                __LINE__);
  for(i = 0; i < NBOATS; i++) {
  ...
  do {
    ...
    SDL_LogVerbose(SDL_LOG_CATEGORY_APPLICATION,
                    "%s l%d A small event loop: just exit on any key or mouse button even\n",
                  __FUNCTION__,
                  __LINE__);
  ...
  for(i = 0; i < NBOATS; i++) {
    ...
    r.x = boatx[i];
    r.y = boaty[i];
    if(SDL_BlitSurface(boat[(boatdir[i] + 1) / 2], NULL, screen, &r) < 0)

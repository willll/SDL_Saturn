...
#define NOTICE(X)	sprintf(text_buffer, "%s",(X));slPrint(text_buffer, slLocate (1,line++));
...
fprintf(stderr, "Converting picture");
		...
		if ( displayfmt == NULL ) {
			fprintf(stderr,
				"Couldn't convert image: %s", SDL_GetError());
			goto done;
		}
...
NOTICE("testwin: moving image");
	for ( i=0; i<=centered; ++i ) {
		...
		if ( SDL_BlitSurface(picture, NULL, screen, &update) < 0 ) {
			fprintf(stderr, "Blit failed: %s", SDL_GetError());
			break;
		}
		if ( flip ) {
			SDL_Flip(screen);
...
int main()
{
	...
	if ( screen == NULL ) {
		sprintf(text_buffer, "Couldn't set %dx%dx%d video mode: %s",
					w, h, desired_bpp, SDL_GetError());
		slPrint(text_buffer, slLocate (1,line++));
		SYS_Exit(1);
	}
	sprintf(text_buffer, "Set %dx%dx%d mode",
			screen->w, screen->h, screen->format->BitsPerPixel);
	slPrint(text_buffer, slLocate (1,line++));
	sprintf(text_buffer, "(video surface located in %s memory)",
			(screen->flags&SDL_HWSURFACE) ? "video" : "system");
	slPrint(text_buffer, slLocate (1,line++));

	if ( screen->flags & SDL_DOUBLEBUF ) {
		sprintf(text_buffer, "Double-buffering enabled");
		slPrint(text_buffer, slLocate (1,line++));
		flip = 1;
	}
